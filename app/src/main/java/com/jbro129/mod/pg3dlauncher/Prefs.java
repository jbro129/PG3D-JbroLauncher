...
public class Prefs
{
    ...
    private Prefs(@NonNull Context paramContext)
    {
        this.sharedPreferences = paramContext.getSharedPreferences("com.jbro129.mod.pg3dlauncher", MODE_PRIVATE);
    }

    private Prefs(@NonNull Context paramContext, @NonNull String paramString)
    {
        this.sharedPreferences = paramContext.getSharedPreferences(paramString, MODE_PRIVATE);
    }

    public static Prefs with(@NonNull Context paramContext)
    {
        if (prefsInstance == null) {
            prefsInstance = new Prefs(paramContext);
        }
        return prefsInstance;
    }

    public static Prefs with(@NonNull Context paramContext, @NonNull String paramString)
    {
        if (prefsInstance == null) {
            prefsInstance = new Prefs(paramContext, paramString);
        }
        return prefsInstance;
    }

    public static Prefs with(@NonNull Context paramContext, @NonNull String paramString, boolean paramBoolean)
    {
        if (paramBoolean) {
            prefsInstance = new Prefs(paramContext, paramString);
        }
    ...
    public static Prefs with(@NonNull Context paramContext, boolean paramBoolean)
    {
        if (paramBoolean) {
            prefsInstance = new Prefs(paramContext);
        }
